...
def naked_twins(values):
	...
	#Iterate through all units, gathering the 'twins' and eliminating them from the rest of the boxes in that unit
...
def search(values):
	...
	#If puzzle cant be solved return False, if all the boxes have been narrowed to one value, return solved puzzle
    if values == False:
        return False
    if all(len(values[s]) == 1 for s in boxes): 
        return values
    #Find the unsolved box with the least number of possibilities. Then, create a copy of the puzzle and solve it by using one of the possibilities through recursion.
